...
def run(url, domain, timeout=10):
    ...
    for path, label in POLICY_FILES:
        ...
        text = resp.text
        if "contact:" in text.lower() or "allow:" in text.lower() or "disallow:" in text.lower() or "google.com" in text.lower() or "contact" in text.lower():
            info("Found Policy File", f"{label} at {target}")
            results["policies_found"].append({"label": label, "url": target})

            # Check security.txt specific rules
    ...
    if not results["policies_found"]:
        info("Policy Audit", f"{Colors.DIM}No policy disclosure files (.well-known/security.txt, etc.) found.{Colors.RESET}")
