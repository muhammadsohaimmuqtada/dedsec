...
class ScanPlan:
    ...
    def load(cls, path: str) -> "ScanPlan":
        ...
        api_specs = [
            _resolve_path(base_dir, item) for item in _unique_strings(discovery.get("api_specs"))
        ]
        template_directories = [
            _resolve_path(base_dir, item) for item in _unique_strings(templates.get("directories"))
        ]
